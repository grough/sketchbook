...
class Coord {
  float width, height, col, row;
  float x, y;

  Coord(int width, int height, int col, int row) {
    this.width = width;
    this.height = height;
    this.col = col;
    this.row = row;
    ...
    if (aspectRatio >= 1) {
      ...
      float scaleW = width;
      float scaleH = height * aspectRatio;
      x = col / scaleW;
      ...
      float scaleH = height;
      float scaleW = width / aspectRatio;
      x = (col - (width - scaleW) / 2) / scaleW;
      x = x < 0 ? 0 : (x > 1 ? 1 : x);
      y = row / scaleH;
    }
  }
